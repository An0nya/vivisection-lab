...
Your granite **overnight sweep finished while the notes still said "(pending)."** It reached **config 372 of a planned 469** before the compute block expired. I analyzed it (`rys-tools/granite/analyze_overnight.py`, output saved to `rys-tools/granite/results/overnight_analysis.txt`) and **rewrote §9 of `granite_sweep_writeup.md`** with real verdicts. New findings, all on the granite page:
...
1. **The granite damping / interference-prune / rescue rounds never ran** (the block died at 372/469). These are *designed and queued* and the results file is append-safe, so they top up without recompute. **The dose-response makes the damping round genuinely interesting now:** is the therapeutic window *wider in α (damping) than in integer copies*? i.e. can a damped 1.5-copy of L10 beat the ×2 ceiling without the ×3 collapse? That's a real, cheap, on-brand next experiment. (Written into §11 of the writeup and §8 of granite.html.)
